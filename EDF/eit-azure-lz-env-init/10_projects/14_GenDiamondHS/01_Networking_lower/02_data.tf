...
data "azurerm_virtual_network" "gendiahs_vnet01" {
  name                = var.diag_vnet_name01
  resource_group_name = var.diag_vnet_rg01
}

data "azurerm_virtual_network" "gendiahs_vnet02" {
  name                = var.diag_vnet_name02
  resource_group_name = var.diag_vnet_rg02
}
########
data "azurerm_key_vault_secret" "bastion-address-prefix" {
  name         = "azbastion-tb-subnet-address-range"
  key_vault_id = data.azurerm_key_vault.genDiamondHSSecrets.id
}
