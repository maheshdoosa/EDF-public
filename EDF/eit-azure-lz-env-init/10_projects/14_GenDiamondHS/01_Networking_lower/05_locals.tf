locals {
  ...
  address_prefix = data.azurerm_key_vault_secret.bastion-address-prefix.value
...
locals {
  diag_settings = {
    "vnet01-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.gendiahs_vnet01.id
    ...
    "vnet02-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.gendiahs_vnet02.id
      law_workspace_id = data.azurerm_key_vault_secret.centaudit-laworkspaceResourceId.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category_enabled
