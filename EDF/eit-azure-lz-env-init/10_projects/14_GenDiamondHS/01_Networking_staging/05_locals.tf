locals {
  ...
  # ############################### UDR loop ######################
  _udr_prefixes  = { for secret in data.azurerm_key_vault_secret.gendiamondhs-udr-prefixes : secret.name => secret.value... }
  udr_prefixes   = { for udr in keys(var.UDR) : udr => split(",", local._udr_prefixes[var.UDR[udr]["kv_secret_udrprefixes"]][0]) }
  _udr_nexthopIp = { for secret in data.azurerm_key_vault_secret.gendiamondhs-udr-nexthop : secret.name => secret.value... }
  ...
  RouteTableIDs       = { for routetable in module.genDiamondHSUdrs : routetable.routetable_name => routetable.routetable_id }
...
locals {
  diag_settings = {
    "testa_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.testa_nsg.id
    ...
    "testb_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.testb_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "replicate_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.replicate_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.gendiastghs_vnet.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
