##Secrets
data "azurerm_key_vault" "genDiamondSSSecrets" {
  name                = var.gendiamondssKvName
  resource_group_name = var.gendiamondssKvRgName
}

################## UDR ##########################################
data "azurerm_key_vault_secret" "gendiamondss-udr-nexthop" {
  for_each     = var.UDR
  name         = each.value.kv_secret_nexthopIP
  key_vault_id = data.azurerm_key_vault.genDiamondSSSecrets.id
}
data "azurerm_key_vault_secret" "gendiamondss-udr-prefixes" {
  for_each     = var.UDR
