# #Secrets
data "azurerm_key_vault" "morwipSecrets" {
  name                = var.morwipKvName
  resource_group_name = var.morwipKvRgName
}

#####################################################################
data "azurerm_key_vault_secret" "morwip-udr-nexthop" {
  for_each     = var.UDR
  name         = each.value.kv_secret_nexthopIP
  key_vault_id = data.azurerm_key_vault.morwipSecrets.id
}

data "azurerm_key_vault_secret" "morwip-udr-prefixes" {
  for_each     = var.UDR
