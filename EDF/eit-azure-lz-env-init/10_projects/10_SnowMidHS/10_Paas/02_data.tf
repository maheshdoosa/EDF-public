#Secrets
data "azurerm_key_vault" "kvsnowmidHSSecrets" {
  name                = var.snowmidHSKvName
  resource_group_name = var.snowmidHSKvRgName
}

data "azurerm_key_vault_secret" "tenantId" {
  name         = "tenantId"
  key_vault_id = data.azurerm_key_vault.kvsnowmidHSSecrets.id
}
...
#### SA with PE ############
data "azurerm_subnet" "secgovHSpesubnet" {
  name                 = var.secgovHSpesubnetName
  virtual_network_name = var.secgovHSVnetName
  resource_group_name  = var.secgovHSNetworkingRgName
}
data "azurerm_storage_account" "secgovHSauxsa" {
  name                = var.secgovHSsa
...
data "azurerm_storage_account" "secgovHSvmDiagsa" {
  name                = var.vmDiagSA
