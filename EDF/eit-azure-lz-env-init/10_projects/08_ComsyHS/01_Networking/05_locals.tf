locals {
  ...
  udr_nexthopIp  = { for udr in keys(var.UDR) : udr => [local._udr_nexthopIp[var.UDR[udr]["kv_secret_nexthopIP"]][0], local._udr_nexthopIp[var.UDR[udr]["kv_secret_nexthopIP"]][0], "", "", ""] }
...
locals {
  diag_settings = {
    "comsy_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.comsy_nsg.id
    ...
    "vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.comsyhs_vnet.id
      law_workspace_id = data.azurerm_key_vault_secret.centaudit-laworkspaceResourceId.value
      diagnostic_log = [{
