...
data "azurerm_subnet" "ControlMHssandSubnet" {
  name                 = var.ControlMHssandSubnetName
  virtual_network_name = var.ControlMHssandVnetName
  resource_group_name  = var.ControlMHssandNetworkingRgName
}
data "azurerm_key_vault_secret" "ControlMHssandvm01PrivateIpAddress" {
  name         = "ControlMHssandvm01PrivateIpAddress"
...
data "azurerm_managed_disk" "ctrlmpecvm01_data_disk" {
  ...
  resource_group_name = var.ControlMHssandvmRgName
  #  depends_on 			= [module.createdisksForctrlmvm01] // need to comment
}
data "azurerm_virtual_machine" "ctrlmpec_vm01" {
  name                = var.ControlMHssandvm01Name
  resource_group_name = var.ControlMHssandvmRgName
  #  depends_on      = [module.createControlMHssandvm01]  // need to comment
}
