locals {
  ...
  # ############################### UDR loop ######################
  _udr_prefixes  = { for secret in data.azurerm_key_vault_secret.controlmhs-udr-prefixes : secret.name => secret.value... }
  udr_prefixes   = { for udr in keys(var.UDR) : udr => split(",", local._udr_prefixes[var.UDR[udr]["kv_secret_udrprefixes"]][0]) }
  _udr_nexthopIp = { for secret in data.azurerm_key_vault_secret.controlmhs-udr-nexthop : secret.name => secret.value... }
  ...
  RouteTableIDs       = { for routetable in module.controlmHSUdrs : routetable.routetable_name => routetable.routetable_id }
...
locals {
  diag_settings = {
    "ctrlm_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.ctrlm_nsg.id
    ...
    "vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.controlmnphs_vnet.id
