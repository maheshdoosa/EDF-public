...
################## UDR ##########################################
data "azurerm_key_vault_secret" "controlmhs-udr-nexthop" {
  for_each     = var.UDR
  name         = each.value.kv_secret_nexthopIP
  key_vault_id = data.azurerm_key_vault.controlmHSSecrets.id
}
data "azurerm_key_vault_secret" "controlmhs-udr-prefixes" {
  for_each     = var.UDR
...
data "azurerm_network_security_group" "ctrlm_nsg" {
  name                = var.ctrlm_nsg_name
...
data "azurerm_virtual_network" "controlmnphs_vnet" {
