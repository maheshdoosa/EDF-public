...
################ Resource Group - Control M np #############
module "create-Target-Rg-For-ControlMHs-np-VM" {
  source            = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-rg?ref=v1.1.19"
  resourceGroupName = var.ControlMHsnpvmRgName
  resourceLocation  = var.ControlMHsResourceLocation
  additionalTags    = module.ControlMHsvmrgTags.tagsResult
}

################ VM01 NIC - Control M np #############
module "createNicForControlMHsnpvm01" {
  source                            = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-vnet/nic-latest?ref=v1.1.19"
  nicName                           = var.ControlMHsnpvm01nicname #should be static
  location                          = var.ControlMHsResourceLocation
  resourceGroupName                 = var.ControlMHsnpvmRgName
  subnetId                          = data.azurerm_subnet.ControlMHsnpSubnet.id
  ...
  ipAddress                         = local.ControlMHsnpvm01PrivateIpAddress
  depends_on                        = [module.create-Target-Rg-For-ControlMHs-np-VM]
}

############### VM01 - Control M np #############
module "createControlMHsnpvm01" {
  source                    = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-vm-rhel-imagebuilder?ref=v1.1.25"
  vmName                    = var.ControlMHsnpvm01Name #this should be static
  computerName              = var.ControlMHsnpvm01Name #should be static across environments
  location                  = var.ControlMHsResourceLocation
  vmResourceGroup           = var.ControlMHsnpvmRgName
  nicIds                    = [module.createNicForControlMHsnpvm01.nicId]
  ...
  EncryptVMsettings         = local.EncryptVMsettingsctrlmnpvm01
  ...
  depends_on                = [module.createNicForControlMHsnpvm01]
}

############### Module to join domain for ControlMHs np VM01 ##########################
module "DomainjoinControlMHsnpvm01" {
  source             = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-vm-extension?ref=v1.1.19"
  name               = "Domainjoin"
  publisher          = "Microsoft.Azure.Extensions"
  type               = "CustomScript"
  handler_version    = "2.1"
  vmId               = module.createControlMHsnpvm01.vmId
  settings           = local.PrepLinuxVMsettings
  protected_settings = local.PrepLinuxVMprotected_settings
  depends_on         = [module.createControlMHsnpvm01]
}

######################### Backup Recovery Service Vault CtrlM HS np ############################
resource "azurerm_backup_protected_vm" "ControlMHsnpvm" {
...
module "attachdisks_CtrlmpecVm01" {
  ...
  caching        = var.caching
  depends_on     = [module.createControlMHsnpvm01, module.createdisksForctrlmvm01]
}
