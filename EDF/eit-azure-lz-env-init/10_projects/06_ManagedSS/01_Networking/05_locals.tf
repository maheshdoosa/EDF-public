locals {
  ...
  # ############################### UDR loop ######################
  _udr_prefixes  = { for secret in data.azurerm_key_vault_secret.mngdss-udr-prefixes : secret.name => secret.value... }
  udr_prefixes   = { for udr in keys(var.UDR) : udr => split(",", local._udr_prefixes[var.UDR[udr]["kv_secret_udrprefixes"]][0]) }
  _udr_nexthopIp = { for secret in data.azurerm_key_vault_secret.mngdss-udr-nexthop : secret.name => secret.value... }
  ...
  RouteTableIDs       = { for routetable in module.mngdssNetworkUdrs : routetable.routetable_name => routetable.routetable_id }
