...
function stageOneConfig
{
    ...
    if($regVariable -lt 1)
    {
        ...
        Get-content -Path C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\1.10.12\Downloads\0\GBRegion.xml | Out-File C:\Temp\GBRegion.xml
...
function stageTwoConfig
{
    ...
    if($regVariable -lt 2)
    {
    	#This is the location of the file which gets downloaded as part of custom script extension
     	$LAPS = "C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\1.10.12\Downloads\0\LAPS.x64.msi"
    	#This will copy the msi locally on the server
     	Invoke-WebRequest $LAPS -OutFile C:\Temp\LAPS.x64.msi
    	#This will install the msi on the server
    	Invoke-Command -scriptblock {Start-Process c:\Temp\LAPS.x64.msi -ArgumentList '-passive' -Wait }
