...
module "avmkvdiag01" {
  ...
  azurerm_key_vault_id       = data.azurerm_key_vault.kvSecrets.id
...
module "azure-private-dns-zone" {
  source                = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-private-dns-zone?ref=v1.0.0"
  private_dns_zone_name = var.private_dns_zone_name
  resource_group_name   = var.kv_resource_group_name
  additionalTags        = module.KVTags.tagsResult
  depends_on            = [module.createkeyvault, module.avmdkvdiag02]
}

############# private DNS zone linking with Vnet ##############
module "azure-private-dns-zone-vnet-link" {
  source                = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-private-dns-zone-vnet-link?ref=v1.0.0"
