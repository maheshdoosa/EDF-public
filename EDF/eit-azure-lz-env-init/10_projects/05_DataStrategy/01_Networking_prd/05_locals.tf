...
locals {
  diag_settings = {
    "uat-vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.datastrgyprdhs_vnet01.id
    ...
    "prd-vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.datastrgyprdhs_vnet02.id
      law_workspace_id = data.azurerm_key_vault_secret.centaudit-laworkspaceResourceId.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category_enabled
