...
locals {
  diag_settings = {
    "vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.eamci_vnet.id
    ...
    "dmzvnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_key_vault_secret.eam-dmzvnet.value
      law_workspace_id = data.azurerm_key_vault_secret.centaudit-laworkspaceResourceId.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category_enabled
