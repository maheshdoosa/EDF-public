...
#####################################################################
data "azurerm_key_vault_secret" "cenadt-udr-nexthop" {
  for_each     = var.UDR
  name         = each.value.kv_secret_nexthopIP
  key_vault_id = data.azurerm_key_vault.cenadtSecrets.id
}

data "azurerm_key_vault_secret" "cenadt-udr-prefixes" {
  for_each     = var.UDR
...
data "azurerm_subnet" "PEPsubnet" {
  name                 = "eit-alz-hs-cenadt-audit-snet-01"
  virtual_network_name = "eit-uks-alz-hs-cenadt-vnet-01"
  resource_group_name  = "eit-uks-alz-hs-cenadt-vnet-rg-01"

  depends_on = [module.AzureMonitorPrivateLinkScope]
}
...
data "azurerm_network_security_group" "centadt_nsg" {
  name                = var.centadt_nsg_name
...
data "azurerm_network_security_group" "pec_nsg" {
  name                = var.pec_nsg_name
  resource_group_name = var.nsg_rg_name
}

data "azurerm_virtual_network" "cenaudit_vnet" {
