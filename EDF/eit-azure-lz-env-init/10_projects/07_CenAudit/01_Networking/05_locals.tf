locals {
  ...
  # ############################### UDR loop ######################
  _udr_prefixes  = { for secret in data.azurerm_key_vault_secret.cenadt-udr-prefixes : secret.name => secret.value... }
  udr_prefixes   = { for udr in keys(var.UDR) : udr => split(",", local._udr_prefixes[var.UDR[udr]["kv_secret_udrprefixes"]][0]) }
  _udr_nexthopIp = { for secret in data.azurerm_key_vault_secret.cenadt-udr-nexthop : secret.name => secret.value... }
  ...
  RouteTableIDs       = { for routetable in module.cenadtNetworkUdrs : routetable.routetable_name => routetable.routetable_id }
...
locals {
  diag_settings = {
    "centadt_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.centadt_nsg.id
    ...
    "pec_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.pec_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.centaudit-laworkspaceResourceId.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.cenaudit_vnet.id
      law_workspace_id = data.azurerm_key_vault_secret.centaudit-laworkspaceResourceId.value
      diagnostic_log = [{
