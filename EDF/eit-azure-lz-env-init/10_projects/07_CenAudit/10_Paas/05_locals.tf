locals {
  ...
  sadnszoneid = data.azurerm_key_vault_secret.sadnszoneid.value
  ...
  diag_settings = {
    ...
    "CenAudit-rsv-diagnostics-hs-CentralAudit" = {
    ...
    "CenAudit-rsv-diagnostics-mgmt-LogWorkSpace" = {
      resource_ids     = module.az-recoveryservicevault.recovery_services_vault.id
...
locals {
  ...
  sa_diag_settings = {
    "sapep-hs-law" = {
      resource_ids     = "${data.azurerm_storage_account.sapep.id}/blobServices/default/"
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId02.value
      diagnostic_log = [
        {
    ...
    "subLogsSA-hs-law" = {
      resource_ids     = "${data.azurerm_storage_account.subLogsSA.id}/blobServices/default/"
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId02.value
      diagnostic_log = [
        {
          category = var.category16
