...
################ Creating RG for tst keyvault ####

module "createTargetRgForTstKV" {
  source            = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-rg?ref=v1.1.19"
  resourceGroupName = var.kv_resource_group_name_tst
  resourceLocation  = var.resource_Location
  additionalTags    = module.KVTags.tagsResult
}
...
module "createkeyvaultTst" {
  source                          = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-keyvault?ref=v1.1.19"
  keyvault_name                   = var.keyvault_name_tst
  resource_Location               = var.resource_Location
  ...
  bypass          = var.bypass
  additionalTags  = module.KVTags.tagsResult
  depends_on      = [module.createTargetRgForTstKV]
  access_policies = local.access_policies
}
...
############# creating private DNS zone for Tst ##############

module "azure-private-dns-zone-tst" {
  source                = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-private-dns-zone?ref=v1.1.19"
  private_dns_zone_name = var.private_dns_zone_name
  resource_group_name   = var.kv_resource_group_name_tst
  additionalTags        = module.KVTags.tagsResult
...
# ############# private DNS zone linking with Tst Vnet ##############
# module "azure-private-dns-zone-vnet-link-tst" {
#   source                = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-private-dns-zone-vnet-link?ref=v1.1.19"
#   vnet_dns_link_name    = var.vnet_dns_link_name
#   resource_group_name   = var.kv_resource_group_name_tst
#   private_dns_zone_name = var.private_dns_zone_name
#   virtual_network_id    = data.azurerm_virtual_network.vnettst.id
...
########### private End Point integrating with Tst DNS Zone ##############

module "azure-private-endpoint-dns-Integrate-Tst" {
  source                         = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-private-endpoint-dns-Integrate?ref=v1.1.19"
  private_dns_zone_name          = var.private_dns_zone_name
  resource_group_name            = var.kv_resource_group_name_tst
  endpoint_name                  = var.endpoint_name
  resource_Location              = var.resource_Location
  subnet_id                      = data.azurerm_subnet.subnettst.id
  endpoint_SC                    = var.endpoint_SC
  private_connection_resource_id = data.azurerm_key_vault.keyvaulttst.id
  is_manual_connection           = var.is_manual_connection
  subresource_names              = var.subresource_names
  #  private_dns_zone_ids           = [data.azurerm_private_dns_zone.dnszonetst.id]
  private_dns_zone_ids = [data.azurerm_key_vault_secret.dnszone.value]
  additionalTags       = module.KVTags.tagsResult
  depends_on           = [module.createkeyvaultTst]
}
/*
####### Module for creating keys for  Azure Disk Encryption ##################3333

module "azure-keyvault-keys" {
source = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-keyvault-keys"
for_each      = var.keys
key_name      = each.value.key_name
key_type      = each.value.key_type
key_size      = each.value.key_size
key_vault_id  = data.azurerm_key_vault.keyvaulttst.id
depends_on    = [module.createkeyvaultTst]
}*/
