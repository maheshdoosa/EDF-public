################ Resource Group - APEX Prod RSV #############
module "createTargetRgForBackup" {
  source = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-rg?ref=v1.1.19"

  resourceGroupName = var.ApexHS_rsv_resourceGroupName
  resourceLocation  = var.ApexHSResourceLocation

  additionalTags = module.ApexHs-Prd-rsv-resourceGroupTags.tagsResult
}

module "ApexHS_recovery_vault" {
  source              = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-recoveryservicevault?ref=v1.1.7"
  rsvname             = var.ApexHS_recovery_vault_name
  resourceLocation    = var.ApexHSResourceLocation
  resourceGroupName   = var.ApexHS_rsv_resourceGroupName
  sku                 = var.sku
  soft_delete_enabled = var.soft_delete_enabled
  # log_analytics_workspace_name = var.RSV_diagnosticsetting_name
  # recovery_services_vault_id   = module.ApexHS_recovery_vault.recovery_services_vault.id
  ...
  additionalTags = module.ApexHs-Prd-rsv-Tags.tagsResult
}
module "rsv-diagsettings" {
  ...
  depends_on                 = [module.ApexHS_recovery_vault]
}
...
module "backup_policy_01" {
  ...
  resourceGroupName         = var.ApexHS_rsv_resourceGroupName
  recovery_vault_name       = var.ApexHS_recovery_vault_name
