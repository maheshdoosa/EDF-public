locals {
  ...
  RouteTableIDs       = { for routetable in module.apexNetworkUdrs : routetable.routetable_name => routetable.routetable_id }
...
locals {
  diag_settings = {
    ...
    "datagw_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.datagw_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "dvweb_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.dvweb_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "dvapp_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.dvapp_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "vnet-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.apexhs_vnet.id
