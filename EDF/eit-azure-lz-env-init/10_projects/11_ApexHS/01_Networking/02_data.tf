# #Secrets
data "azurerm_key_vault" "apexSecrets" {
  name                = var.apexKvName
  resource_group_name = var.apexRgName
}

#####################################################################
data "azurerm_key_vault_secret" "apex-udr-nexthop" {
  for_each     = var.UDR
  name         = each.value.kv_secret_nexthopIP
  key_vault_id = data.azurerm_key_vault.apexSecrets.id
}

data "azurerm_key_vault_secret" "apex-udr-prefixes" {
  for_each     = var.UDR
...
data "azurerm_network_security_group" "datagw_nsg" {
  name                = var.datagw_nsg_name
  resource_group_name = var.nsg_rg_name
}

data "azurerm_network_security_group" "dvweb_nsg" {
  name                = var.dvweb_nsg_name
  resource_group_name = var.nsg_rg_name
}
data "azurerm_network_security_group" "dvapp_nsg" {
  name                = var.dvapp_nsg_name
  resource_group_name = var.nsg_rg_name
}
...
data "azurerm_virtual_network" "apexhs_vnet" {
