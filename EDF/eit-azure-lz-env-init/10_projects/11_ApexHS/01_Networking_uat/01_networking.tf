module "apexuatNetworkResourceGroup" {
  source            = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-rg?ref=v1.1.19"
  for_each          = local.rgs
  resourceGroupName = each.key
  resourceLocation  = each.value.location
  additionalTags    = module.apexuatresourceGroupTags.tagsResult
}
...
resource "azurerm_virtual_network" "vnets" {
  ...
  depends_on          = [module.apexuatNetworkResourceGroup]
  tags                = module.apexuatresourceGroupTags.tagsResult
}
...
module "apexuatNetworkUdrs" {
  ...
  tags       = module.apexuatUdrTable.tagsResult
...
resource "azurerm_subnet_route_table_association" "assingUDRs" {
  ...
  depends_on = [module.apexuatNetworkUdrs]
}
