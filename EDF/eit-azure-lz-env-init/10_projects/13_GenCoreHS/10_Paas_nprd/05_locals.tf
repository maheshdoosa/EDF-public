locals {
  ...
  diag_settings = {
    ...
    "Gencore-NonProd-rsv-diagnostics-hs-CentralAudit" = {
    ...
    "Gencore-NonProd-rsv-diagnostics-mgmt-LogWorkSpace" = {
      resource_ids     = module.az-recoveryservicevault.recovery_services_vault.id
...
locals {
  ...
  sa_diag_settings = {
    ...
    "scriptsSA-hs-law" = {
      resource_ids     = "${data.azurerm_storage_account.scriptsSA.id}/blobServices/default/"
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId02.value
      diagnostic_log = [
        {
          category = var.category19
