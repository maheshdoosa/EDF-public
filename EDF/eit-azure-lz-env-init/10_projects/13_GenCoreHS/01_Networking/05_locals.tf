locals {
  ...
  # ############################### UDR loop ######################
  _udr_prefixes  = { for secret in data.azurerm_key_vault_secret.gencore-udr-prefixes : secret.name => secret.value... }
  udr_prefixes   = { for udr in keys(var.UDR) : udr => split(",", local._udr_prefixes[var.UDR[udr]["kv_secret_udrprefixes"]][0]) }
  _udr_nexthopIp = { for secret in data.azurerm_key_vault_secret.gencore-udr-nexthop : secret.name => secret.value... }
  ...
  RouteTableIDs       = { for routetable in module.gencoreNetworkUdrs : routetable.routetable_name => routetable.routetable_id }
  ...
  asg = { for asg in keys(var.asg) : asg => var.asg[asg] if var.asg[asg].enable == true }
...
locals {
  diag_settings = {
    ...
    "av_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.av_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "sccm_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.sccm_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "tmr_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.tmr_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "pep02_nsg-diag-setting" = {
      resource_ids     = data.azurerm_network_security_group.pep02_nsg.id
      law_workspace_id = data.azurerm_key_vault_secret.laworkspaceId01.value
      diagnostic_log = [{
        category = var.category1
        enabled  = var.category1enabled
    ...
    "vnet01-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.gencorehs_vnet01.id
    ...
    "vnet02-diag-setting_logs" = {
      resource_ids     = data.azurerm_virtual_network.gencorehs_vnet02.id
      law_workspace_id = data.azurerm_key_vault_secret.centaudit-laworkspaceResourceId.value
      diagnostic_log = [{
        category = var.category3
        enabled  = var.category_enabled
