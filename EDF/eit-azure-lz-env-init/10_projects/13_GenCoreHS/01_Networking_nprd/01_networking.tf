########### Resource Group ##################
module "gencoreNetworkResourceGroup" {
...
resource "azurerm_virtual_network" "vnets" {
  ...
  depends_on          = [module.gencoreNetworkResourceGroup]
...
module "gencoreNetworkUdrs" {
  ...
  tags       = module.gencoreUdrTable.tagsResult
...
resource "azurerm_subnet_route_table_association" "assingUDRs" {
  ...
  depends_on = [module.gencoreNetworkUdrs]
}
...
module "createVNetPeeringTMR" {
  count = var.vnet_peering["peer-hub-spoke-01"]["enable"] == true ? 1 : 0
  providers = {
    azurerm.src = azurerm.hub
    azurerm.dst = azurerm.spoke
  }
  source                 = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-vnetpeering-multiple-subs?ref=v1.1.19"
  vNet1ResourceGroupName = var.vnet_peering["peer-hub-spoke-01"]["vnet1rg"]
  vNet2ResourceGroupName = var.vnet_peering["peer-hub-spoke-01"]["vnet2rg"]

  peerNameVNet12 = var.vnet_peering["peer-hub-spoke-01"]["peer1name"]
  peerNameVNet21 = var.vnet_peering["peer-hub-spoke-01"]["peer2name"]
  vNet1Name      = var.vnet_peering["peer-hub-spoke-01"]["vnet1name"]
  vNet1Id        = data.azurerm_virtual_network.hubVNet[0].id
  vNet2Name      = var.vnet_peering["peer-hub-spoke-01"]["vnet2name"]
  vNet2Id        = local.vnet_ids[var.vnet_peering["peer-hub-spoke-01"]["vnet2name"]]
...
module "azure-application_security_group" {
  source              = "git::ssh://git@github.com/edfenergy/eit-azure-terraform-modules.git//azure-asg?ref=v1.1.19"
  for_each            = local.asg
  name                = each.key
  location            = each.value.location
  resource_group_name = each.value.rg
  additionalTags      = module.asgtags.tagsResult
  depends_on          = [module.gencoreNetworkResourceGroup]
}
...
module "diagsettings" {
  ...
  depends_on                 = [module.nsgs, azurerm_virtual_network.vnets]
}
