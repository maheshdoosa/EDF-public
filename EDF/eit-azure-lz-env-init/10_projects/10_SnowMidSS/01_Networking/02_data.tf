# #Secrets
data "azurerm_key_vault" "sssecgovSecrets" {
  name                = var.sssecgovKvName
  resource_group_name = var.sssecgovKvRgName
}

#####################################################################
data "azurerm_key_vault_secret" "sssecgov-udr-nexthop" {
  for_each     = var.UDR
  name         = each.value.kv_secret_nexthopIP
  key_vault_id = data.azurerm_key_vault.sssecgovSecrets.id
}

data "azurerm_key_vault_secret" "sssecgov-udr-prefixes" {
  for_each     = var.UDR
